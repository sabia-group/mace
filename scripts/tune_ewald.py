...
def main():
    parser = argparse.ArgumentParser(
        description="Tune Ewald parameters for a system from an extxyz file."
    )
    ...
    # Tune Ewald
    print("Tuning Ewald parameters...")
    smearing, Ewald_params, _ = tune_ewald(
        charges=charges,
        cell=cell,
        positions=positions,
        cutoff=args.cutoff,
    ...
    print("Ewald tuning complete.")
    print(f"Smearing parameter: {smearing}")
    print(f"Ewald parameters: {Ewald_params}")

    # Save results to JSON
    results = {
        "smearing": smearing.item() if torch.is_tensor(smearing) else smearing,
        **{k: (v.item() if torch.is_tensor(v) else v) for k, v in Ewald_params.items()},
    }

    with open(args.output, "w") as f:
        json.dump(results, f, indent=2)
    print(f"Saved Ewald tuning results to {args.output}")
