...
def neighbor_ranges(num_neighbor: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    Args:
        num_neighbor (torch.Tensor): 1D tensor of length N_atoms,
            where num_neighbor[i] is the number of neighbors of atom i.
            Must be non-negative integers.

    Returns:
        (start, end) (tuple of torch.Tensor):
            - start: 1D tensor of length N_atoms, start indices (inclusive)
            - end:   1D tensor of length N_atoms, end indices (exclusive)

    Example:
        >>> num_neighbor = torch.tensor([3, 2, 4])
        ...
        # Which means:
        # Atom 0 neighbors are at indices [0:3]
        # Atom 1 neighbors are at indices [3:5]
        # Atom 2 neighbors are at indices [5:9]
    """
    assert num_neighbor.dim() == 1, f"num_neighbor must be 1D, got {num_neighbor.shape}"
    assert (
        num_neighbor >= 0
    ).all(), "num_neighbor must contain only non-negative integers"

    # Start indices: prepend 0, drop last element, then cumulative sum
    start = torch.cat(
        (
            torch.tensor([0], device=num_neighbor.device, dtype=num_neighbor.dtype),
            num_neighbor[:-1],
        )
    )
    start = torch.cumsum(start, dim=0)

    # End indices: cumulative sum of neighbor counts
    end = torch.cumsum(num_neighbor, dim=0)
